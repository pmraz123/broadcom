...
# --------------------------------------------------------------------------------------------------
        ...
        self.api_getEnterpriseEdges = "/portal/rest/enterprise/getEnterpriseEdges"
        self.api_getEnterpriseEdges_JSON_BODY = {
                            "enterpriseId": int(self.VCO_TENANT_ID),
                            "with": ["site", "links", "configuration",
                                    "cloudServices", "wan",
                                    "nvsFromEdge", "certificates", "licenses"]
                    }
        # this is to filter header and get important data
        self.api_getEnterpriseEdges_header = ['id' ]
        # this is to "transform" header to user readable format
        self.api_getEnterpriseEdges_header_readable = ['id' ]
        
        # here are all keys for above API - October 2023 
        # keys:
        ...
        self.api_getEnterprise_JSON_BODY= {
        "id": 13,
        "jsonrpc": "2.0",
...
# used 
    ...
    def post_api(self, API, data):
        ...
        if self.PROXY_STATE == True:
            response = requests.post(f"{self.base_url}" , verify=False, json=data, proxies=PROXIES, headers=headers, timeout=600)
